export enum SubmitAndPay2Content {
  h3 = "Declaration",
  p1 = "I understand that proceedings for contempt of court may be brought against anyone who makes, or causes to be made, a false statement in a document verified by a statement of truth without an honest belief in its truth.",
  p21 = "The applicant believes that the facts stated in this form and any continuation sheets are true.",
  p22 = "is authorised by the applicant to sign this statement.",
  formHint = "This option should only be selected if the legal representative is signing the form on behalf of the applicant.",
  formLabelAgreeWithStatement = "I agree with this statement",
  h3Hidden1 = "Next step - Submit the application",
  h3Hidden2 = "If the applicant has applied for Help with Fees, you should continue to process their application online.",
  h3Hidden3 = "You will still have to pay the fee. You can apply for a refund by contacting the court and providing the applicant's Help with Fees reference.",
  h3Hidden4 = "Application fee due to be paid",
  h3Hidden5 = "Final checks",
  h3Hidden6 = "C100 draft",
  hiddenP1 = "After you have submitted your application, you will be asked to pay the application fee.",
  hiddenP2 = "Download and check the application before you proceed. Make sure that all details are correct.",
  formLabelDraftConsent = "Draft Document",
  formLabelDraftDocumentWelsh = "Draft Document (Welsh)",
  link1 = "Draft_C100_application.pdf",
  link2 = "Draft_C100_application_welsh.pdf",
  continue = "Continue",
}
