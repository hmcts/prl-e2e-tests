...
export class AmendChildDetails1Page extends EventPage {
  ...
  private readonly dobDayField: Locator = this.page.locator("#dateOfBirth-day").first();
  private readonly dobMonthField: Locator = this.page.locator(
    "#dateOfBirth-month").first();
;
  private readonly dobYearField: Locator = this.page.locator("#dateOfBirth-year").first();
  ...
  async assertPageContents(): Promise<void> {
    ...
    await expect(
        this.page.locator(Selectors.h3, { hasText: AmendChildDetailsRevised1Content.h3AddNewChild }).first() // Isolates the first heading globally
    ).toBeVisible();

    await expect(
        this.page.locator(Selectors.GovukFormLabel, { hasText: AmendChildDetailsRevised1Content.formLabelFirstName }).first()
    ).toBeVisible();
    await expect(
  ...
  async fillInFields({ c100ChildGender, under18 }: FillInFieldsOptions) {
    ...
    await this.firstNameField.fill(AmendChildDetailsRevised1Content.childFirstName);
    await this.lastNameField.fill(AmendChildDetailsRevised1Content.childLastName);
