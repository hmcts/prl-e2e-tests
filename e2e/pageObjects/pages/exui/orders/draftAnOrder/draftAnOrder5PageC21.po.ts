...
export class DraftAnOrder5PageC21 extends Base {
   ...
   private readonly headings: string[] = [
    ...
    constructor(page: Page, accessibilityTest = false, isUploadOrder = false, solicitorCaseCreateType?: string) {
        super(page);
    }

    async assertPageContentsToBeVisible(): Promise<void> {
            for (const heading of this.headings) {
            await expect(this.page.getByRole("heading", { name: heading }), `Expected heading "${heading}" to be visible`).toBeVisible();
            }

            for (const labelsAndText of this.labelsAndText) {
            await expect(this.page.getByText(labelsAndText), `Expected text "${labelsAndText}" to be visible` ).toBeVisible();
            }
            ...
            await expect(this.page.getByRole("button", { name: navigationButton, exact: true }), `Expected navigation button "${navigationButton}" to be visible`).toBeVisible();
            }
        }
    ...
    async fillOrderDetails(details: OrderDetails): Promise<void> {
        ...
        await step(`orderApprovedAtHearing = "${details.orderApprovedAtHearing}"`, async () => {
            await this.page.locator(radioIds.wasTheOrderApprovedAtHearing_Yes).check();
            await expect(this.page.locator(radioIds.wasTheOrderApprovedAtHearing_Yes)).toBeChecked();
        });

        // --- 2) At which hearing was the order approved? (if provided) ---
        if (details.hearingApprovedAt !== undefined) {
            await step(`hearingApprovedAt = "${details.hearingApprovedAt}"`, async () => {
            const hearingSelect = this.page.getByRole("combobox").first();
            await hearingSelect.selectOption({ label: details.hearingApprovedAt });
        ...
        await step(`judgeTitle = "${details.judgeTitle}"`, async () => {
            const radio = this.page.getByRole("radio", { name: details.judgeTitle, exact: true });
            await radio.check();
            await expect(radio).toBeChecked();
        });
        ...
        await step(`judgeFullName = "${details.judgeFullName}"`, async () => {
            const input = this.page.getByLabel("Judge's full name (Optional)", { exact: true });
        ...
        if (details.legalAdviserFullName !== undefined) {
            await step(`legalAdviserFullName = "${details.legalAdviserFullName}"`, async () => {
            const input = this.page.getByLabel("Full name of Justices' Legal Adviser (Optional)", {
                exact: true,
            });
        ...
        await step(`orderAboutAllChildren = "${details.orderAboutAllChildren}"`, async () => {
            const question = this.page.getByText("Is the order about all the children?", { exact: false });
            const container = question.locator("..");  

            await this.page.locator(radioIds.isTheOrderAboutAllChildren_Yes).check();
            await expect(this.page.locator(radioIds.isTheOrderAboutAllChildren_Yes)).toBeChecked();
        });
