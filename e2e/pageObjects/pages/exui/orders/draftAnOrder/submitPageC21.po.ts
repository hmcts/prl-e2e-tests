...
export class SubmitPageC21 extends Base {
  ...
  private readonly expectedLabelValues: LabelValuePair[] = [
    { label: "What do you want to do?", value: "Upload an order" },
    {
      ...
      value: "Blank order or directions (C21)",
    },
    { label: "Is the order by consent?", value: "No" },
    {
      label: "Select the type of order",
      value: "Blank order or directions (C21): application refused",
    },
    { label: "Was the order approved at a hearing?", value: "Yes" },
    {
      label: "At which hearing was the order approved?",
      value: "No hearings available",
    },
    {
      label: "Select or amend the title of the Judge or magistrate",
      value: "Her Honour Judge",
    },
    { label: "Judge's full name", value: "Judge PC" },
    {
      label: "Full name of Justices' Legal Adviser",
      value: "Legal Adviser PC",
    },
    { label: "Date order made", value: "10 Oct 2025" },
    { label: "Is the order about all the children?", value: "Yes" },
  ];
  ...
  async assertPageContentsToBeVisible(): Promise<void> {
    ...
    for (const nav of ["Submit", "Previous"]) {
      await test.step(`Button should be visible and enabled: "${nav}"`, async () => {
        ...
        await expect(
          button,
          `Expected "${nav}" button to be visible`,
        ).toBeVisible();
        await expect(
          button,
          `Expected "${nav}" button to be enabled`,
        ).toBeEnabled();
      });
    }
    // Then check Cancel link
    await test.step(`Cancel link should be visible and clickable`, async () => {
      const cancelLink = this.page.getByRole("link", {
        name: "Cancel",
        exact: true,
      });
      ...
      await expect(
        cancelLink,
        `Expected "Cancel" link to be visible`,
      ).toBeVisible();
      await expect(
        cancelLink,
        `Expected "Cancel" link to be enabled`,
      ).toBeEnabled();
    });
  }
  ...
  async assertExpectedLabelValuesPresent(
    ...
    const firstRow = this.page
      .locator("table.form-table tr.ng-star-inserted")
      .first();
    ...
    for (const { label, value } of expected) {
      await test.step(`Check answers row: "${label}" -> "${value}"`, async () => {
        ...
        await expect(row, `Row not found for label: "${label}"`).toHaveCount(
          1,
          { timeout: 10000 },
        );
  ...
  async assertChangeLinksPresentForLabels(): Promise<void> {
    ...
    console.log(
      `Checking Change links for ${this.labelsThatMustHaveChangeLink.length} labels...`,
    );
    ...
    console.log(
      `✓ Successfully verified Change links for all ${this.labelsThatMustHaveChangeLink.length} labels`,
    );
  }
