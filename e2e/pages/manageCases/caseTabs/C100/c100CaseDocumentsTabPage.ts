...
export class C100CaseDocumentsTabPage {
  public static async c100CaseDocumentsTabPageManageDocuments(
    ...
    await Promise.all([
      ...
      expect(
        uploadedDocsSection
          .getByText(C100CaseDocumentsTabContent.text16_document)
          .first(),
      ).toBeVisible(),
      expect(
        uploadedDocsSection.getByText(
      ...
      expect(uploadedDocsSection.getByText(documentCategory)).toBeVisible(),
      expect(uploadedDocsSection.getByText(documentParty)).toBeVisible(),
    ]);
  }
}
