...
class CaseManagerAPI {
    ...
    constructor(apiContext: APIRequestContext) {
        this.baseUrl = urlConfig.serviceUrl;
        this.apiContext = apiContext;
    }

    // Get access token
    private async getAccessToken(user: { email: string, password: string }): Promise<string | undefined> {
        try {
    ...
    // Create a case
    public async createCase(caseName: string = 'e2e UI Test', user: { email: string, password: string }): Promise<string | undefined> {
    ...
    public async updateCase(caseName: string = 'e2e Test', caseID: string, caseDataJson: any) {
        try {
            const docDetail = await this.apiRequest(`${this.baseUrl}/testing-support/test-document`, systemUpdateUser);
    ...
    public async giveAccessToCase(caseID: string, user: { email: string, password: string }, role: string) {
        const data = {
            email: user.email,
