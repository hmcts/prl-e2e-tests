...
test.describe("Complete the Send to Gatekeeper task as Case Manager for FL401 Solicitor case tests.", () => {
  ...
  scenarios.forEach((params: SendToGateKeeperJourneyParams) => {
    test(`Complete Send to Gatekeeper with ${params.sendToGateKeeperParams.judgeOrLegalAdviser ? params.sendToGateKeeperParams.judgeOrLegalAdviser : "no specific gatekeeper"} as gatekeeper @regression @accessibility`, async ({
      page,
    }) => {
