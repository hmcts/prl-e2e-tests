...
caseTypes.forEach((caseType) => {
  test.describe(`Create a hearing request for a ${caseType} case`, () => {
    test.beforeEach(
      async ({ caseWorker, manageCasesEventUtils, navigationUtils }) => {
        const caseRef = (
          await manageCasesEventUtils.submitTSSolicitorCase(caseType)
        ).caseRef;
    ...
    test("Complete Task - Create Hearing Request with accessibility test. @nightly @regression @accessibility", async ({
      ...
      await hearingsPage.assertHearingAdded();
    });
  });
});
