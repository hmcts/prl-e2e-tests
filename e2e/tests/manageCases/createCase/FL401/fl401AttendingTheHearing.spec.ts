...
test.describe("FL401 Create case respondent’s behaviour tests @manageCases", (): void => {
  ...
  Not Accessibility testing,
  Yes error Messaging,
  Yes to All Options, @crossbrowserManageCases`, async({page}): Promise<void> => {
    await Fl401AttendingTheHearing.fl401AttendingTheHearing({
      page: page,
      accessibilityTest: false,
      errorMessaging: true,
...
test(`Test the accessibility of the FL401 Attending The Hearing Journey, @accessibilityManageCases`, async({page}): Promise<void> => {
  await Fl401AttendingTheHearing.fl401AttendingTheHearing({
    page: page,
